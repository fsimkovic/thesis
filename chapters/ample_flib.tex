...
An analysis of each fragment with regards to its \gls{rmsd} to the target structure and a classification into true and false positives (by \gls{rmsd}-cutoff of 1.5\AA) further highlighted the best contact pairs for fragment picking to be top-$L$ short-, medium- and long-range contacts from the METAPSICOV STAGE 1 predictions (\cref{fig:ample_FLIB_fragtps}). The classification of fragments into true and false positives also highlights the localisation of true positives according to secondary structure elements in the target structure. 

Noticeably, no fragment library covers last 20\% of the target sequences with some not covering more than 50\%. ... why? :-O

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{ample_FLIB_fragtps.pdf}
	\caption[]{FLIB fragment distribution of true (blue) and false (red) positive fragments per residue position in the target sequence. True and false positivity was assessed by a \gls{rmsd}-cutoff of 1.5\AA\ compared to the native structure. The predicted secondary structure of each target sequence is given at the top: \textalpha-helices (red), \textbeta-strands (blue), and loops (gray). The fragment frequency in each bin is presented using a log scale.}
...
% - satisfaction of contact information <-- freq dist for no contacts satisfied <-- either a bug or a serious problem with discrepancies of contact information and satisfaction
