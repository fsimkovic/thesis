...
\begin{figure}[H]
    ...
    \caption{Schematic of inference of covariance signal originating from evolutionary pressure in protein tertiary structures and encoded in its family's sequence alignment (adapted from \cite{Simkovic2017-xs}).}
...
\begin{equation}
    EC(i,j)=FN(i,j)-\frac{FN(i,\dot)FN(\dot,j)}{FN(\dot,\dot)}
    \label{eq:average_product_correction}
\end{equation}
\equations{Average Product Correction}

Lastly, To avoid overfitting of the data, regularisation is typically applied \cite{Hopf2017-pp}. Furthermore, all sequences in the input alignment are commonly re-weighted by a redundacy describing factor, since a key assumption of \cref{eq:potts_model} rests on the basis that all sequences evolved independently \cite{Ekeberg2013-ay}.
