...
An unbiased selection of 27 non-redundant protein targets was selected using the following protocol (\cref{table:appendix_dataset_predictors}).
...
\section{Residue-residue contact prediction}
The prediction of residue-residue contact information was essential to the work presented in this thesis. In the following section, an overview is given to outline the procedures of the different (meta-)predictors employed.
...
\end{equation}
\equations{Multiple Sequence Alignment depth}
...
\begin{equation} 
    Prec=\frac{TruePositives}{TruePositives-FalsePositives}
...
\subsubsection{Long-range contact precision} \label{sec:methods_longrange_satisfaction}
The long-range contact pair satisfaction score is computed identically to the precision of sets of contact pairs (\cref{sec:methods_contact_map_prec}). However, the satisfaction score is computed solely for long-range contacts ($>23$ residues sequence separation).
